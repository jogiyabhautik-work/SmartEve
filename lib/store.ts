...
// In-Memory global singleton for mock/fallback mode
...
// Initial seed data
const now = new Date();
const baseTime = new Date(now.getTime() - 5 * 60 * 1000); // 5 minutes ago for realistic demo
...
// Global in-memory storage instance across server/client sessions
class MemoryStore {
  ...
  getEventByJoinCode(code: string): EventData | null {
    return Object.values(this.state.events).find(
...
export const memoryStore = globalForStore.storeInstance || new MemoryStore();
if (process.env.NODE_ENV !== "production") globalForStore.storeInstance = memoryStore;
